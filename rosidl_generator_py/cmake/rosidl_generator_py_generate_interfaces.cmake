...
set(_generated_cpython_sources "")
...
list(APPEND _generated_py_files
  ...
  list(APPEND _generated_py_files
    "${_output_path}/${_parent_folder}/experimental/detail/${_module_name}__cpython_binding.hpp")
  # The binding implementation is compiled into the package's bindings
  # library (a regular shared library); the CPython extension links its own
  # bindings library and those of its dependencies, never other extensions.
  list(APPEND _generated_cpython_sources
    "${_output_path}/${_parent_folder}/experimental/detail/${_module_name}__cpython_binding.cpp")
endforeach()

# The package-level pybind11 module (generated by the generator).
list(APPEND _generated_py_files
  "${_output_path}/msg/experimental/${PROJECT_NAME}__module.cpp")

# Visibility control header (export/import macros for the bindings library).
string(TOUPPER "${PROJECT_NAME}" PROJECT_NAME_UPPER)
configure_file(
  "${rosidl_generator_py_TEMPLATE_DIR}/rosidl_generator_py__visibility_control.hpp.in"
  "${_output_path}/msg/rosidl_generator_py__visibility_control.hpp"
  @ONLY
)
list(APPEND _generated_py_files
  "${_output_path}/msg/rosidl_generator_py__visibility_control.hpp")
...
set(target_dependencies
  ...
  "${rosidl_generator_py_TEMPLATE_DIR}/msg__experimental.cpp.em"
...
# Export the generated binding dir so downstream packages (and their
# experimental pybind11 extensions) can include cross-package binding headers.
target_include_directories(${_target_name_lib} INTERFACE
  "$<BUILD_INTERFACE:${CMAKE_CURRENT_BINARY_DIR}/rosidl_generator_py>"
...
# ---------------------------------------------------------------------------
# Experimental pybind11 bindings: a per-package bindings library (regular
# shared library with the binding implementations) plus a thin CPython
# extension (_bindings) that links its own bindings library and those of its
# dependencies — never other extensions. Replaces the legacy Python
# experimental message classes.
# ---------------------------------------------------------------------------
set(_module_file "${_output_path}/msg/experimental/${PROJECT_NAME}__module.cpp")

find_package(pybind11_vendor REQUIRED)
find_package(pybind11 REQUIRED)
find_package(rosidl_runtime_cpython REQUIRED)

# Bindings library: compiled binding implementations (handle classes,
# register functions). A regular shared library, not a CPython extension.
set(_target_name_cpython "${rosidl_generate_interfaces_TARGET}__rosidl_generator_cpython")
add_library(${_target_name_cpython} SHARED ${_generated_cpython_sources})
target_compile_definitions(${_target_name_cpython} PRIVATE
  "ROSIDL_GENERATOR_PY_BUILDING_DLL_${PROJECT_NAME}"
)
target_include_directories(${_target_name_cpython} PUBLIC
  "$<BUILD_INTERFACE:${CMAKE_CURRENT_BINARY_DIR}/rosidl_generator_py>"
  "$<INSTALL_INTERFACE:include/${PROJECT_NAME}>"
  ${rosidl_runtime_cpython_INCLUDE_DIRS}
)
...
foreach(_pkg_name ${rosidl_generate_interfaces_DEPENDENCY_PACKAGE_NAMES})
  ...
  target_link_libraries(${_target_name_cpython} PRIVATE
    ${${_pkg_name}_TARGETS__rosidl_generator_cpython})
endforeach()
add_dependencies(${_target_name_cpython} ${rosidl_generate_interfaces_TARGET}${_target_suffix})

# Thin CPython extension: only the module .cpp (PYBIND11_MODULE). Links its
# own bindings library and those of its dependencies.
pybind11_add_module(_bindings "${_module_file}")
target_link_libraries(_bindings PRIVATE
  ${_target_name_cpython}
  ${rosidl_generate_interfaces_TARGET}__rosidl_generator_cpp
...
foreach(_pkg_name ${rosidl_generate_interfaces_DEPENDENCY_PACKAGE_NAMES})
  target_link_libraries(_bindings PRIVATE
    ${${_pkg_name}_TARGETS__rosidl_generator_cpython})
endforeach()
add_dependencies(_bindings ${_target_name_cpython})
set_target_properties(_bindings PROPERTIES
  LIBRARY_OUTPUT_DIRECTORY $<1:${_output_path}/msg/experimental>
)

if(NOT rosidl_generate_interfaces_SKIP_INSTALL)
  install(TARGETS _bindings
    DESTINATION "${PYTHON_INSTALL_DIR}/${PROJECT_NAME}/msg/experimental"
  )
  install(TARGETS ${_target_name_cpython}
    EXPORT export_${_target_name_cpython}
    ARCHIVE DESTINATION lib
    LIBRARY DESTINATION lib
    RUNTIME DESTINATION bin)
  rosidl_export_typesupport_targets("__rosidl_generator_cpython" "${_target_name_cpython}")
  ament_export_targets(export_${_target_name_cpython})
  # Install the binding headers (double-nested, matching rosidl_generator_cpp)
  # so downstream packages can include cross-package binding headers.
  install(DIRECTORY "${_output_path}/msg/"
    DESTINATION "include/${PROJECT_NAME}/${PROJECT_NAME}/msg"
