...
///
/// The macro imports the PyCapsule from _rosidl_runtime_cpython and caches the
...
/// @brief Capsule name exported by rosidl_runtime_cpython._raw_buffer.
#define ROSIDL_RUNTIME_PY_RAW_BUFFER_CAPSULE_NAME \
  "rosidl_runtime_cpython._raw_buffer._C_API"
...
static const RawBuffer_CAPI_t * _rosidl_runtime_cpython_CAPI = NULL;

/// @brief Import the C API from _rosidl_runtime_cpython.  Call once at init.
/// @return 0 on success, -1 on failure (Python exception is set).
static inline int
RawBuffer_ImportCAPI(void)
{
  if (_rosidl_runtime_cpython_CAPI != NULL) {
    return 0;
  }
  // PyCapsule_Import returns void * directly — not a PyObject *.
  _rosidl_runtime_cpython_CAPI = (const RawBuffer_CAPI_t *)PyCapsule_Import(
    ROSIDL_RUNTIME_PY_RAW_BUFFER_CAPSULE_NAME, 0);
  return (_rosidl_runtime_cpython_CAPI != NULL) ? 0 : -1;
}
...
RawBuffer_FromRegion(const rosidl_memory_region_t * region)
{
  ...
  return _rosidl_runtime_cpython_CAPI->FromRegion(region);
}
